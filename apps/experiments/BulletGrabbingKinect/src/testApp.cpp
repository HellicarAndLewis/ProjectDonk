...
ofVec2f transformPoint(ofVec2f pos) {
//	pos *= ofGetWindowSize();
	float zoom = 0.66;
	pos.x = ofMap(pos.x, 0, 1, ofGetWidth()*(1.f-zoom), ofGetWidth()*zoom, true);
	pos.y = ofMap(pos.y, 0, 1, ofGetHeight()*(1.f-zoom), ofGetHeight()*zoom, true);
	return pos;
}
...
void testApp::touchDown(ofTouchEventArgs &touch) {
	...
	mousePressed(pos.x, pos.y, touch.id);
}
...
void testApp::touchMoved(ofTouchEventArgs &touch) {
	...
	ofVec2f pos(touch.x, touch.y);
	pos = transformPoint(pos);
	mouseDragged(pos.x, pos.y, touch.id);
}

void testApp::touchUp(ofTouchEventArgs &touch) {
	ofVec2f pos(touch.x, touch.y);
	pos = transformPoint(pos);
	mouseReleased(pos.x, pos.y, touch.id);
}