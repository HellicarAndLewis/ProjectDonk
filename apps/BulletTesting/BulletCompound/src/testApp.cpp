...
void testApp::setup() {
	...
	bd = bullet.createSphere(ofVec3f(0, 100, 0), 30, 1);
	/*for (int i=0; i<10; i++) {
		ofVec3f pos(ofRandom(-50, 50), ofRandom(0, 150), ofRandom(-50, 50));
		bullet.createSphere(pos, ofRandom(8, 15), 1);
	}*/
	
	cube = new ofxBulletCompoundBody();
	cube->world = bullet.world;
	cube->compoundShape = new btCompoundShape();
	
	int size = 50;
	
	btTransform startTrans;
	startTrans.setIdentity();
	startTrans.setOrigin(btVector3(0,100,0));
	
	btTransform localTrans;
	localTrans.setIdentity();
	localTrans.setOrigin(btVector3(0,-size,0));
	btCollisionShape * plateShape = new btBoxShape(btVector3(size, 1, size));
	cube->compoundShape->addChildShape(localTrans, plateShape);
	
	localTrans.setIdentity();
	localTrans.setOrigin(btVector3(0,0,size));
	btCollisionShape * plateShapeA = new btBoxShape(btVector3(size, size, 1));
	cube->compoundShape->addChildShape(localTrans, plateShapeA);
	
	localTrans.setIdentity();
	localTrans.setOrigin(btVector3(0,0,-size));
	btCollisionShape * plateShapeD = new btBoxShape(btVector3(size, size, 1));
	cube->compoundShape->addChildShape(localTrans, plateShapeD);
	
	localTrans.setIdentity();
	localTrans.setOrigin(btVector3(size,0,0));
	btCollisionShape * plateShapeB = new btBoxShape(btVector3(1, size, size));
	cube->compoundShape->addChildShape(localTrans, plateShapeB);
	
	localTrans.setIdentity();
	localTrans.setOrigin(btVector3(-size,0,0));
	btCollisionShape * plateShapeC = new btBoxShape(btVector3(1, size, size));
	cube->compoundShape->addChildShape(localTrans, plateShapeC);
	
	cube->createCompoundBody(1, startTrans);
	
	bullet.world->addRigidBody(cube->body);
	bullet.rigidBodies.push_back(cube);
}
