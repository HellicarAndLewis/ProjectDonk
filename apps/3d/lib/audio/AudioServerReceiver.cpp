...
#include "AudioServerReceiver.h"

AudioServerReceiver::AudioServerReceiver() {
	connected = false;
	numChannels = 6; // default value
	signals = NULL;
}

void AudioServerReceiver::connect(int port) {
	osc.setup(port);
	connected = true;
}

void AudioServerReceiver::update() {
	if(!connected) {
...
int AudioServerReceiver::getNumChannels() {
	return numChannels;
}
float AudioServerReceiver::getVolume(int i) {
	// check bounds
...
void AudioServerReceiver::draw(float x,float y,float w, float h) {
