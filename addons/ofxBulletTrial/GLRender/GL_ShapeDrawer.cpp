...
GL_ShapeDrawer::ShapeCache*		GL_ShapeDrawer::cache(btConvexShape* shape)
{
	...
	if(!sc)
	{
		...
		for(int i=0;i<ni;i+=3)
		{
			...
			const btVector3		v1  = pv[ti[1]]-pv[ti[0]];
			const btVector3		v2  = pv[ti[2]]-pv[ti[0]];
												   
			const btVector3		nrm = v1.cross(v2).normalized();
...
void GL_ShapeDrawer::drawOpenGL(btScalar* m, const btCollisionShape* shape, const btVector3& color,int	debugMode,const btVector3& worldBoundsMin,const btVector3& worldBoundsMax, btVector4 bodyColor)
{
...
//			glEnable(GL_TEXTURE_GEN_T); // ???
		...
		glColor4f(bodyColor.x(), bodyColor.y(), bodyColor.z(), bodyColor.w());
		//glColor4f(0.1, 1, 0.1, 0.3);
...
void		GL_ShapeDrawer::drawShadow(btScalar* m,const btVector3& extrusion,const btCollisionShape* shape,const btVector3& worldBoundsMin,const btVector3& worldBoundsMax)
{
	...
	else
	{
		...
		if (shape->isConvex())
		{
			...
			{			
				const btScalar		d = sc->m_edges[i].n[0].dot(extrusion);//btDot(sc->m_edges[i].n[0],extrusion);
				const btScalar      dd = sc->m_edges[i].n[1].dot(extrusion);
				if((d * dd) < 0)
				{
					const int			q=	d<0?1:0;
